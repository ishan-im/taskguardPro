			...
			<div
				...
				<div className='flex items-center justify-between w-full'>
					...
					<div className='flex items-center gap-4'>
						<button
							className={`Button1 ${
								doneTodo ? 'opacity-50' : 'opacity-100'
							} p-[18px] py-2.5 bg-purple-500 rounded-[20px] justify-center items-center gap-1.5 flex cursor-pointer`}
							disabled={doneTodo}
							onClick={handleModalToggle}>
							<div className='VuesaxBoldEdit2 w-4 h-4 justify-center items-center flex'>
								<img src={Edit} className='w-4 h-4  inline-block' alt={Edit} />
							</div>
						</button>
						<button
